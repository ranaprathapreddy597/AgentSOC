...
export const ReasoningPanel: React.FC<ReasoningPanelProps> = ({ hypothesis, simulationValid }) => {
  if (!hypothesis) {
    return (
      <div className="cyber-glass-card rounded-2xl p-6 border border-slate-800/80 shadow-2xl flex flex-col items-center justify-center text-center min-h-[220px]">
        ...
        <h3 className="text-xs font-semibold text-slate-400">NCE AI Engine Idle</h3>
        <p className="text-[11px] text-slate-600 mt-1">Awaiting incident telemetry for real-time hypothesis generation.</p>
      </div>
    );
  }
  ...
  const getConfidenceBarColor = (score: number) => {
    if (score >= 0.7) return 'bg-gradient-to-r from-rose-600 to-rose-400';
    if (score >= 0.3) return 'bg-gradient-to-r from-amber-600 to-amber-400';
